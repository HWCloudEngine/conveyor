...
class SignalResponder(stack_user.StackUser):
    ...
    def _encrypt(self, in_value):
        value = None
        try:
            value = crypt.heat_encrypt(in_value)
        except Exception:
            LOG.error("encrypt failed, return unencrypted data")
            return in_value
    ...
    def _get_ec2_signed_url(self, signal_type=SIGNAL):
        ...
        request['params']['Signature'] = self._encrypt(
            signer.generate(request))
