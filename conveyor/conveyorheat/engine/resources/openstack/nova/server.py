...
class Server(stack_user.StackUser, sh.SchedulerHintsMixin,
    ...
    def handle_create(self):
        ...
        if key_name and self.context.trustor_user_id:
            instance_meta.update({
                'op_svc_userid': self.context.trustor_user_id
            })
    ...
    def needs_replace_failed(self):
        ...
        with self.client_plugin().ignore_not_found:
            server = self.client().servers.get(self.resource_id)
            return server.status in ('DELETED', 'SOFT_DELETED')
    ...
    def _check_networks_num(self, networks=[]):
        if cfg.CONF.FusionSphere.pubcloud:
            if len(networks) > 1:
                msg = _('The length of Property "%(networks)s" '
                        'is greater than one. The maximum length is'
                        ' one.') % dict(networks=self.NETWORKS)
                raise exception.StackValidationFailed(message=msg)
    ...
    def validate(self):
        ...
        self._check_networks_num(networks=networks)
