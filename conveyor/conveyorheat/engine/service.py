...
from conveyor.conveyorheat.engine.cfn import template as cfntemplate
from conveyor.conveyorheat.engine import clients
from conveyor.conveyorheat.engine import environment
from conveyor.conveyorheat.engine import event as evt
from conveyor.conveyorheat.engine.hot import functions as hot_functions
from conveyor.conveyorheat.engine import parameter_groups
from conveyor.conveyorheat.engine import properties
from conveyor.conveyorheat.engine import resources
from conveyor.conveyorheat.engine import service_software_config
from conveyor.conveyorheat.engine import service_stack_watch
from conveyor.conveyorheat.engine import stack as parser
from conveyor.conveyorheat.engine import stack_lock
...
from conveyor.conveyorheat.objects import snapshot as snapshot_object
...
class EngineService(service.Service):
    ...
    def identify_stack(self, cnxt, stack_name):
        ...
        if uuidutils.is_uuid_like(stack_name):
            ...
            s = stack_object.Stack.get_by_name(cnxt, stack_name)
        LOG.info(_LI('Identifying stack %s'), stack_name)
        if s:
    ...
    def list_resource_types(self,
        ...
        """
        def filter(resource_types):
            if not cfg.CONF.FusionSphere.filter_resource:
                return resource_types

            supported_resources = []
            if os.path.isfile(cfg.CONF.support_resources_conf):
                try:
                    with open(cfg.CONF.support_resources_conf, 'r') as f:
                        supported_res_dict = jsonutils.load(f)
                        supported_resources.extend(
                            [res_type.strip(os.linesep)
                             for res_type in supported_res_dict
                             if res_type]
                        )
                except Exception:
                    # do nothing
            ...
            if supported_resources:
                return [res for res in resource_types
                        if res in supported_resources]
            else:
                return resource_types

        return filter(resources.global_env().get_types(
            cnxt,
    ...
    def resource_signal(self, cnxt, stack_identity, resource_name, details,
        ...
        def _resource_signal(stack, rsrc, details, need_check):
            LOG.info("signaling resource %s:%s" % (stack.name, rsrc.name))
        ...
        try:
            ...
            stack = parser.Stack.load(cnxt, stack=s, use_stored_context=False)
            rsrc = stack[resource_name]
            if not rsrc or rsrc.type() != "Huawei::FusionSphere::HAPolicy":
                LOG.info("needs to load stored context to signal %s"
                         % resource_name)
                raise e
