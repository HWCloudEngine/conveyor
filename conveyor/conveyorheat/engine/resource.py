...
from conveyor.conveyorheat.engine.cfn import template as cfn_tmpl
...
from conveyor.conveyorheat.engine.hot import template as hot_tmpl
...
class Resource(object):
    ...
    def load(cls, context, resource_id, is_update, data):
        from conveyor.conveyorheat.engine import stack as stack_mod
    ...
    def handle_metadata_reset(self):
        """Rewrite metadata to the database.

        The resource does not work if its metadata hangs in an unavailable
        value, such as metadata 'scaling_in_progress' of scaling_policy or
        scaling_group resource, the value is always true if engine restarts
        while scaling.

        Adding this interface for resource plugins is used to reset metadata
        to an available value to make sure the resources work again.
        """
        pass
    ...
    def is_service_available(cls, context):
        ...
        if (cls.default_client_name is None or
                cfg.CONF.FusionSphere.pubcloud):
            return True
