...
class StackUpdate(object):
    ...
    def check_update_constraint(self):
        """preview update the stack and check the constraint"""
        replaced_resources_keys = self.preview()['replaced']
        LOG.info("update stack replace resource %s" %
                 replaced_resources_keys)

        def check_resource_replace():
            for key in replaced_resources_keys:
                current_res = self.existing_stack.resources[key]
                if not current_res.is_allow_replace():
                    # subnet can not replace in public cloud
                    raise exception.UpdateReplace()

        try:
            check_resource_replace()
        except exception.UpdateReplace:
            msg = '%s has unsupport replace resource' \
                  % replaced_resources_keys
