...
def get_system_ca_file():
    ...
    for ca in ca_path:
        ...
        if os.path.exists(ca):
            ...
            return ca
    LOG.error("System ca file could not be found.")
...
class HTTPClient(object):
    ...
    def strip_endpoint(self, location):
        def _format_url(url):
            ...
            # ignore default port number 80 in http url
            if url_parsed.scheme == 'http' and \
                    url_parsed.port == 80:
                return url.replace(':80', '')
            # ignore default port number 443 in https url
            elif url_parsed.scheme == 'https' and \
                    url_parsed.port == 443:
        ...
        # format url, and remove default port number
        endpoint_formatted = _format_url(self.endpoint)
        location_formatted = _format_url(location)

        if location_formatted.lower().startswith(endpoint_formatted.lower()):
            return location_formatted[len(endpoint_formatted):]
        else:
