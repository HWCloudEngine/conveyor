...
from conveyor.conveyorheat.engine.notification import stack as notification
from conveyor.conveyorheat.engine import parameter_groups as param_groups
...
from conveyor.conveyorheat.engine import template as tmpl
from conveyor.conveyorheat.engine import update
from conveyor.conveyorheat.objects import raw_template as raw_template_object
...
class Stack(collections.Mapping):
    ...
    def __init__(self, context, stack_name, tmpl,
        ...
        if use_stored_context:
            ...
            if not cfg.CONF.FusionSphere.pubcloud:
        ...
        if self.user_creds_id:
            if cfg.CONF.FusionSphere.pubcloud:
                creds_obj = ucreds_object.UserCreds.get_by_id(
                    self.user_creds_id)
                creds_info_temp = creds_obj.obj_to_primitive()[
                ...
                creds['tenant_id'] = self.context.tenant_id
                creds['iam_assume_token'] = \
                    self.context.clients.client('iam').auth.assume_role()
                creds['auth_token'] = creds['iam_assume_token']
                creds['trustor_user_id'] =\
                    creds_info_temp.get('trustor_user_id')

                return common_context.RequestContext.from_dict(creds)
            else:
                creds_obj = ucreds_object.UserCreds.get_by_id(
                    self.user_creds_id)
                # Maintain request_id from self.context so we retain
                # traceability in situations where servicing a request
                # requires switching from the request context to the stored
                # context
                creds = creds_obj.obj_to_primitive()[
                    "versioned_object.data"]
                creds['request_id'] = self.context.request_id
                # We don't store roles in the user_creds table, so disable the
                # policy check for admin by setting is_admin=False.
