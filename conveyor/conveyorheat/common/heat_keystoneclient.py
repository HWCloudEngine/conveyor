...
class KeystoneClientV3(object):
    ...
    def delete_trust(self, trust_id):
        ...
        trust_client = (self.admin_client
                        if cfg.CONF.FusionSphere.pubcloud else self.client)
        try:
            trust_client.trusts.delete(trust_id)
        except kc_exception.NotFound:
            pass
        except kc_exception.EmptyCatalog:
            trust_manager = self.client.trusts
            if cfg.CONF.FusionSphere.pubcloud:
                base_url = self.v3_endpoint + '/OS-TRUST-EXT'
            else:
                base_url = self.v3_endpoint + '/OS-TRUST'
            kwargs = {
                'base_url': base_url,
            ...
            url = trust_manager.build_url(dict_args_in_out=kwargs)
            trust_manager.client.delete(url)
    ...
    def url_for(self, **kwargs):
        ...
        if cfg.CONF.FusionSphere.pubcloud:
            url = self.admin_client.service_catalog.url_for(**kwargs)
            admin_project_id = self.admin_client.auth_ref.project_id
            user_project_id = self.context.tenant_id
            return url.replace(admin_project_id, user_project_id)
