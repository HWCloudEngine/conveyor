...
from conveyor.conveyorheat.api.aws import exception as aws_exception
...
groupwatch_api_opts = [
    ...
    cfg.IntOpt('bind_port', default=8060,
...
groupwatch_group = cfg.OptGroup('groupwatch_api')
cfg.CONF.register_group(groupwatch_group)
cfg.CONF.register_opts(groupwatch_api_opts,
                       group=groupwatch_group)
...
api_hw_opts = [
    cfg.StrOpt('ssl_version', default='SSLv23',
               help=_('ssl version for api'),
               deprecated_group='DEFAULT'),
    cfg.StrOpt('ciphers',
               default=('EECDH+ECDSA+AESGCM:EECDH+aRSA+AESGCM:'
                        'EECDH+ECDSA+SHA384:EECDH+ECDSA+SHA256:'
                        'EECDH+aRSA+SHA384:EECDH+aRSA+SHA256:'
                        'EECDH:EDH+aRSA:RSA+AES:!aNULL:!eNULL:'
                        '!LOW:!3DES:!MD5:!EXP:!PSK:!SRP:!DSS:!RC4'),
               help=_('ciphers for api'),
               deprecated_group='DEFAULT')
]
api_hw_group = cfg.OptGroup('FusionSphere')
cfg.CONF.register_group(api_hw_group)
cfg.CONF.register_opts(api_hw_opts, group=api_hw_group)
...
class WritableLogger(object):
    ...
    def write(self, msg):
        try:
            parse = urlparse.urlparse(msg)
            query = dict(urlparse.parse_qsl(parse.query))
        except Exception:
            return

        # replace sensitive message info in url such as Signature,
        # SignatureMethod and AWSAccessKeyId
        for key in ('Signature', 'SignatureMethod', 'AWSAccessKeyId'):
            if key in query:
                query[key] = '***'

        parameter_list = list(parse)
        parameter_list[4] = urllib.urlencode(query)

        log = urlparse.ParseResult(*parameter_list).geturl()
        self.LOG.log(self.level, log.strip("\n"))
...
class Server(object):
    ...
    def configure_socket(self, old_conf=None, has_changed=None):
        ...
        """
        def get_ssl_version():
            if not cfg.CONF.FusionSphere.ssl_version:
                return ssl.PROTOCOL_SSLv23

            ssl_version = cfg.CONF.FusionSphere.ssl_version.lower()
            if ssl_version == 'tlsv1_2':
                return ssl.PROTOCOL_TLSv1_2
            elif ssl_version == 'tlsv1_1':
                return ssl.PROTOCOL_TLSv1_1
            elif ssl_version == 'tlsv1_0':
        ...
        if wrap_sock:
            self.sock = ssl.wrap_socket(self._sock,
                                        ...
                                        ciphers=cfg.CONF.FusionSphere.ciphers,
    ...
    def run_server(self):
        ...
        try:
            eventlet.wsgi.server(
                ...
                log=WritableLogger(self._logger),
