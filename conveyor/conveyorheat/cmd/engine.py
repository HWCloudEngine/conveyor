...
def main():
    ...
    cfg.CONF(sys.argv[1:], project='conveyor', prog='conveyor-heat',
             version=version.version_string())
    # cfg.CONF(project='heat', prog='heat-engine',
    #          version=version.version_info.version_string())
    # change log format string for logging to the heat-engine file
    cfg.CONF.logging_context_format_string = \
        cfg.CONF.logging_context_format_string.replace('conveyor', 'conveyor-heat')
    cfg.CONF.logging_default_format_string = \
        cfg.CONF.logging_default_format_string.replace('conveyor', 'conveyor-heat')
    logging.setup(cfg.CONF, 'conveyor')
    ...
    from conveyor.conveyorheat.engine import service as engine  # noqa

    profiler.setup('conveyor-heat', utils.get_hostid())
    # profiler.setup('conveyor-heat', cfg.CONF.host)
    gmr.TextGuruMeditation.setup_autorun(version)
    srv = engine.EngineService(utils.get_hostid(), rpc_api.ENGINE_TOPIC)
    # srv = engine.EngineService(cfg.CONF.host, rpc_api.ENGINE_TOPIC)
    workers = cfg.CONF.num_engine_workers or 1
    if not workers:
