...
class FloatIps(base.resource):
    ...
    def extract_floatingips(self, floatingip_ids):
        ...
        for floatingip in floatingip_objs:
            ...
            network_cls = networks.NetworkResource(self.context,
                                                collected_resources=self._collected_resources,
                                                collected_dependencies=self._collected_dependencies)

            net_res =  network_cls.extract_nets([floating_network_id], with_subnets=True)
            
            # refresh collected resource in order to add network and subnet resource
            self._collected_resources = network_cls.get_collected_resources();
            self._collected_dependencies = network_cls.get_collected_dependencies()
