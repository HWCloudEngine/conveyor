    ...
    def _extract_resource(self, context, stack_id):
        ...
        for name, resource in template.get('resources',{}).iteritems():
            ...
            if self.is_file_type(t1):
                resource['content'] = self._extract_resource(context,self.get_resources(sub_res_list, t1)[0].physical_resource_id)
                resource['id'] = self.get_resources(sub_res_list, t1)[0].physical_resource_id
                continue
            ...
            for sub in self.get_resources(sub_res_list, t1):
                ...
                for descendant in self._list_sub_resources(context,sub_res_id):
