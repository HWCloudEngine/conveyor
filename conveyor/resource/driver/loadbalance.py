    ...
    def extract_loadbalanceVip(self, vip_id, pool_name):
        ...
        if listener_ids:
            ...
            LoadbalanceListener(self.context,
                                ...
                                collected_dependencies= \
                                self._collected_dependencies)
            listener_driver.extract_loadbalanceListeners(listener_ids, vip_id, vip_name)
...
class LoadbalancePool(base.resource):
    ...
    def extract_loadbalancePool(self, pool_id):
        ...
        if vip_id:
            ...
            self._collected_dependencies = vip_driver.get_collected_dependencies()
        ...
        if subnet_id:
            ...
            self._collected_resources = newtwork_driver.get_collected_resources()
            self._collected_dependencies = newtwork_driver.get_collected_dependencies()
        ...
        if member_ids:
            ...
            self._collected_resources = lb_member_driver.get_collected_resources()
            self._collected_dependencies = lb_member_driver.get_collected_dependencies()
        ...
        if healthmonitor_ids:
            ...
            # update collect resource
            self._collected_resources = lb_healthmonitor_driver.get_collected_resources()
            self._collected_dependencies = lb_healthmonitor_driver.get_collected_dependencies()
...
class LoadbalanceListener(base.resource):
    ...
    def extract_loadbalanceListener(self, listener_id, vip_id, vip_name):
        ...
        try:
            listener = self.neutron_api.show_listener(self.context, listener_id, vip_id)
        except Exception as e:
            _msg='Create LB listener resource error: %s' % e
            LOG.error(_msg)
            raise exception.ResourceExtractFailed(reason=_msg)
        ...
        listener_name = 'loadbalanceListener_%d' % self._get_resource_num(listener_type)

        listener_res = resource.Resource(listener_name, listener_type,
                                    listener_id, properties=properties)
...
class LoadbalanceMember(base.resource):
    ...
    def extract_loadbalanceMember(self, member_id, pool_name):
        ...
        member_name = 'loadbalancePoolMember_%d' % self._get_resource_num(member_type)
    ...
    def extract_loadbalanceMembers(self, member_ids, pool_name):
        ...
        if not member_ids or not pool_name:
            ...
            _msg='Create LB member resource error: member id or pool is null.'
            raise exception.InvalidInput(reason=_msg)
        try:
...
class LoadbalanceHealthmonitor(base.resource):
    ...
    def extract_loadbalanceHealthmonitor(self, healthmonitor_id):
        ...
        try:
            healthmonitor = self.neutron_api.show_health_monitor(self.context, healthmonitor_id)
        except Exception as e:
            _msg='Create LB health monitor resource error: %s' % e
            LOG.error(_msg)
            raise exception.ResourceExtractFailed(reason=_msg)
        ...
        healthmonitor_res = resource.Resource(healthmonitor_name, healthmonitor_type,
                                       healthmonitor_id,
