...
def find_config(config_path):
    ...
    :raises: `conveyor.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "conveyor", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/conveyor/%s" % config_path,
    ]
...
def cinderdir():
    import conveyor
    return os.path.abspath(conveyor.__file__).split('conveyor/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'conveyor.api.ec2.cloud:' \
     conveyor.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See conveyor.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo conveyor-rootwrap %s' % CONF.rootwrap_config


def brick_get_connector_properties(multipath=False, enforce_multipath=False):
    """wrapper for the brick calls to automatically set
    the root_helper needed for conveyor.
...
def require_driver_initialized(driver):
    ...
    if not driver.initialized:
        ...
        LOG.error(_LE("Conveyor driver %s not initialized"), driver_name)
